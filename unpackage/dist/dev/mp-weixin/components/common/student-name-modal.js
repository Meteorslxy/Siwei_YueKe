  ...
  methods: {
    ...
    open: function open() {
      ...
      try {
        ...
        var isAutoGeneratedNickname = nickname.startsWith('用户') || nickname === '微信用户' || nickname.includes('默认昵称') || /^用户\d+$/.test(nickname) // 匹配"用户"后跟数字的模式
        ;

        console.log('当前昵称:', nickname || '无昵称', isAutoGeneratedNickname ? '(自动生成的昵称)' : '');
