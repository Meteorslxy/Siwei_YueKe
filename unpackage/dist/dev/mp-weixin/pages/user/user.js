              ...
              case 4:
                ...
                try {
                  ...
                  if (userInfo && (userInfo._id || userInfo.uid)) {
                    ...
                    nickname = userInfo.nickname || userInfo.nickName; // 如果用户已设置昵称或已标记过，则不显示弹窗
                    if (hasSetStudentName || nickname && nickname.trim() !== '') {
                      // 确保hasSetStudentName标记已设置
                      if (!hasSetStudentName && nickname) {
                        console.log('用户已有昵称但未标记，自动设置hasSetStudentName标记');
